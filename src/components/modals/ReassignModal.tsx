      ...
      <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-cyan-500/40 rounded-2xl shadow-2xl overflow-hidden glass-panel-glow">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="text-[11px] font-mono text-cyan-800 dark:text-cyan-400 font-bold uppercase tracking-wider">
                  UNSPSC Official Catalog (Column L)
                </span>
              ...
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Modify Column L Commodity Code</h3>
            </div>
          </div>
        ...
        <div className="p-5 space-y-4">
          <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 space-y-1 text-xs">
            <span className="text-slate-500 dark:text-slate-400">Target Line Item:</span>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search Column L code or commodity description..."
                value={search}
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
            {searchResults.map((rec) => {
              const isSelected = selectedRecord?.commodityCode === rec.commodityCode || (!selectedRecord && item.unspsc_code === rec.commodityCode);
              return (
                <div
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-xs text-slate-900 dark:text-white">{rec.commodityTitle}</span>
                    <span className="text-[11px] font-mono font-bold px-2 py-0.5 rounded bg-cyan-100 dark:bg-slate-800 text-cyan-800 dark:text-cyan-400 border border-cyan-300 dark:border-cyan-800/60">
                      Col L: {rec.commodityCode}
                    </span>
                  </div>
                  <div className="mt-1 flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400">
                    <span>Class: {rec.classTitle} ({rec.classCode})</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-300">
                      Bucket: {rec.coreBucket}
                    </span>
                  </div>
                  <div className="text-[10px] text-slate-400 font-mono mt-0.5">
                    Segment: {rec.segmentTitle}
                  </div>
                </div>
              );
            })}
        ...
        <div className="flex items-center justify-end space-x-2 px-5 py-3.5 border-t border-slate-100 dark:border-cyan-500/20 bg-slate-50/80 dark:bg-slate-950/60">
          ...
          >
            ...
            <span>Apply Column L Mapping</span>
          </button>
        </div>
      </div>
    </div>
