...
const updateVendorConsolidation = (
  ...
  if (!items || items.length === 0) return { ...init };
...
const updateCategorySavings = (
  ...
  const spend = Number(
    items
      .reduce((acc, opp) => acc + (opp.current_spend_inr_cr || (opp.current_spend * 83.8) / 10000000), 0)
      .toFixed(2)
  );
  const savings = Number(
    items
      .reduce((acc, opp) => acc + (opp.est_savings_inr_cr || (opp.est_savings * 83.8) / 10000000), 0)
      .toFixed(2)
  );
  const pct = spend > 0 ? Number(((savings / spend) * 100).toFixed(1)) : 0;
  return { ...init, spendInrCr: spend, savingsInrCr: savings, savingsPct: pct };
